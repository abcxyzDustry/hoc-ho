...
import './models/StudentCardPhoto.js';
...
mongoose.connect(MONGO_URI)
  .then(async () => {
    ...
    try {
      // Dọn index geo "rác" còn sót lại trên Partner (từ schema cũ, khi latitude/longitude
      // từng bị đánh index 2d/2dsphere) — đây là nguyên nhân lỗi "Can't extract geo keys"
      // khi đối tác bật công tắc nhận đơn / cập nhật vị trí.
      await dropStaleGeoIndexes(Partner, 'Partner');

      await Order.createIndexes();
      console.log('✅ Đã đảm bảo index (gồm 2dsphere cho geo-matching) được tạo');
    } catch (idxErr) {
      console.error('⚠️ Lỗi tạo index:', idxErr.message);
    }
